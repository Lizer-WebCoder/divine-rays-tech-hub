-- Fix: profiles table missing "role" column
-- Run this in SQL Editor if you got: column "role" does not exist

-- Ensure profiles has the columns we need
...
-- Optional: allow only customer/agent (skip if constraint already exists)
do $$
begin
  alter table public.profiles
...
select 'profiles.role ready' as status;
