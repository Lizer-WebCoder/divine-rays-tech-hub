/**
 * Divine Rays Tech Hub — Ticketing System v5.1
 * Fixed registration links + hardened handlers
...
function loginCustomer(email, password) {
  ...
  var user = users.find(function (u) {
    return u.role === 'customer' &&
      u.email && u.email.toLowerCase() === String(email).toLowerCase().trim() &&
      u.password === password;
  });
  if (!user) return null;
...
function loginAgent(username, password) {
  ...
  var user = users.find(function (u) {
    return u.role === 'agent' &&
      u.username && u.username.toLowerCase() === String(username).toLowerCase().trim() &&
      u.password === password;
  });
  if (!user) return null;
...
function openCustomerTicket(id) {
  ...
  if (detail) {
    detail.innerHTML = '<h3>' + escapeHtml(ticket.title) + '</h3>' +
  ...
  if (list) {
    list.innerHTML = publicComments.length === 0
      ...
      : publicComments.slice().reverse().map(function (c) {
          return '<div class="comment"><div class="comment-header">' +
            '<span>' + escapeHtml(c.author) + '</span>' +
            '<span>' + formatDate(c.createdAt) + (c.statusChange ? ' · → ' + c.statusChange : '') + '</span>' +
            '</div><div class="comment-body">' + escapeHtml(c.text) + '</div></div>';
        }).join('');
  }
}
...
function setupCustomer() {
  ...
  if (btnTrack) {
    btnTrack.addEventListener('click', function () {
      ...
      var publicComments = (ticket.comments || []).filter(function (c) { return !c.internal; });
      box.hidden = false;
      box.innerHTML = '<h3 style="margin-bottom:0.55rem">' + escapeHtml(ticket.title) + '</h3>' +
        ...
        (publicComments.length === 0
          ...
          : publicComments.slice().reverse().map(function (c) {
              return '<div class="comment" style="margin-bottom:0.45rem">' +
                '<div class="comment-header"><span>' + escapeHtml(c.author) + '</span>' +
                '<span>' + formatDate(c.createdAt) + (c.statusChange ? ' · → ' + c.statusChange : '') + '</span></div>' +
...
function renderTicketList() {
  ...
  else if (listFilter.mode === 'unassigned') tickets = tickets.filter(function (t) {
    return !t.assignedTo && t.status !== 'Resolved' && t.status !== 'Closed';
  });
...
function setupAgent() {
    ...
    btnSave.addEventListener('click', function () {
      ...
      if (ticket.assignedTo !== newAgent) {
        ...
        ticket.comments.push({
          ...
          author: session.name, createdAt: new Date().toISOString(), internal: true
        });
      ...
      if (ticket.status !== newStatus) {
        ...
        ticket.comments.push({
          ...
          author: session.name, createdAt: new Date().toISOString(),
          internal: false, statusChange: newStatus
        });
  ...
  if (commentForm) {
    commentForm.addEventListener('submit', function (e) {
      ...
      ticket.comments = ticket.comments || [];
      ticket.comments.push({
        text: text, author: session.name,
        createdAt: new Date().toISOString(), internal: isInternal
      });
      ticket.updatedAt = new Date().toISOString();
      saveTickets(tickets);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Create account / sign in links (event delegation so they always work)
  document.addEventListener('click', function (e) {
    var target = e.target;
    if (!target || !target.id) return;

    if (target.id === 'show-register-customer') {
      e.preventDefault();
      showForm('register-customer');
    } else if (target.id === 'show-login-customer') {
      e.preventDefault();
      showForm('login-customer');
    } else if (target.id === 'show-register-agent') {
      e.preventDefault();
      showForm('register-agent');
    } else if (target.id === 'show-login-agent') {
      e.preventDefault();
      showForm('login-agent');
    }
  });
