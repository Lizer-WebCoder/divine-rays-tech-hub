/**
 * Divine Rays Tech Hub v6.9 — search, filters, export, polish
...
var currentProfile=null, currentCustTicketId=null, currentTicketId=null, currentView='dashboard', listFilter={mode:'all',q:'',status:'',priority:''}, nameCache={}, allTicketsCache=[];
...
function detailMetaHtml(t,names,isAgent){names=names||{};var bits=[];bits.push('<span class="meta-chip"><span class="meta-k">ID</span> <span class="ticket-id">'+esc(t.ticket_number)+'</span></span>');bits.push('<span class="meta-chip"><span class="meta-k">Status</span> <span class="badge '+sc(t.status)+'">'+esc(t.status)+'</span></span>');bits.push('<span class="meta-chip"><span class="meta-k">Priority</span> <span class="badge '+sc(t.priority)+'">'+esc(t.priority||'—')+'</span></span>');if(t.category)bits.push('<span class="meta-chip"><span class="meta-k">Category</span> '+esc(t.category)+'</span>');if(isAgent){bits.push('<span class="meta-chip"><span class="meta-k">Requester</span> '+esc(names[t.requester_id]||'Customer')+'</span>');bits.push('<span class="meta-chip"><span class="meta-k">Assigned</span> '+esc(t.assigned_to?(names[t.assigned_to]||'Agent'):'Unassigned')+'</span>');}bits.push('<span class="meta-chip"><span class="meta-k">Created</span> '+fmt(t.created_at)+'</span>');if(t.updated_at)bits.push('<span class="meta-chip"><span class="meta-k">Updated</span> '+fmt(t.updated_at)+'</span>');return '<div class="detail-meta">'+bits.join('')+'</div>';}
function commentsHtml(comments,isAgent){if(!comments||!comments.length)return '<p class="empty-state" style="padding:1rem">No activity yet.</p>';return comments.map(function(c){if(c.is_internal&&!isAgent)return '';var author=(c.author&&c.author.full_name)||'User';var cls='comment'+(c.is_internal?' internal':'');var tag=c.is_internal?' <span class="internal-tag">internal</span>':'';return '<div class="'+cls+'"><div class="comment-header"><strong>'+esc(author)+'</strong>'+tag+'<span>'+fmt(c.created_at)+'</span></div><div class="comment-body">'+esc(c.body)+'</div></div>';}).join('');}
...
window.showForm=showForm;window.switchLoginTab=switchLoginTab;window.showApp=showApp;window.showLoginScreen=showLoginScreen;window.DR={getProfile:function(){return currentProfile;},sb:function(){return sb;},toast:toast,esc:esc,fmt:fmt,sc:sc,detailMetaHtml:detailMetaHtml,commentsHtml:commentsHtml,fetchTickets:fetchTickets,fetchProfileNames:fetchProfileNames,getListFilter:function(){return listFilter;},setAllTickets:function(t){allTicketsCache=t||[];},getAllTickets:function(){return allTicketsCache;},getNameCache:function(){return nameCache;},setNameCache:function(m){nameCache=Object.assign(nameCache,m||{});},openTicket:function(id){return openTicket(id);},renderStats:function(){return renderStats();}};
...
async function renderMyTickets(){if(!currentProfile)return;var c=document.getElementById('my-tickets-list');if(!c)return;c.innerHTML='<div class="empty-state"><p>Loading…</p></div>';var tickets=await fetchTickets({requesterId:currentProfile.id});if(!tickets.length){c.innerHTML='<div class="empty-state"><p>No tickets yet.</p></div>';return;}c.innerHTML=tickets.map(function(t){return '<div class="ticket-card" data-id="'+t.id+'"><div><h4>'+esc(t.title)+'</h4><div class="ticket-meta"><span class="ticket-id">'+esc(t.ticket_number)+'</span><span>'+esc(t.category||'')+'</span><span>'+fmt(t.updated_at||t.created_at)+'</span></div></div><div class="badges"><span class="badge '+sc(t.priority)+'">'+esc(t.priority||'')+'</span><span class="badge '+sc(t.status)+'">'+esc(t.status)+'</span></div></div>';}).join('');c.querySelectorAll('.ticket-card').forEach(function(card){card.addEventListener('click',function(){openCustomerTicket(card.getAttribute('data-id'));});});}
async function openCustomerTicket(id){var t=await fetchTicketById(id);if(!t)return;currentCustTicketId=id;showCustomerTab('detail');var d=document.getElementById('cust-ticket-detail');if(d){d.innerHTML='<h3>'+esc(t.title)+'</h3>'+detailMetaHtml(t,{},false)+'<div class="detail-description">'+esc(t.description)+'</div>';}var cl=document.getElementById('cust-comments-list');if(cl){cl.innerHTML='<p class="empty-state" style="padding:0.5rem">Loading…</p>';var comments=await fetchComments(id);cl.innerHTML=commentsHtml(comments,false);}}
function setupCustomer(){
  ...
  var bt=document.getElementById('btn-track');if(bt)bt.addEventListener('click',async function(){if(!requireCloud())return;var num=document.getElementById('track-id').value.trim();if(!num){toast('Enter a ticket ID','error');return;}var t=await fetchTicketByNumber(num);var box=document.getElementById('track-result');if(!box)return;box.hidden=false;if(!t){box.innerHTML='<p style="color:var(--danger)">Ticket not found. Check the ID (e.g. DR-1001).</p>';return;}box.innerHTML='<h3>'+esc(t.title)+'</h3>'+detailMetaHtml(t,{},false)+'<div class="detail-description">'+esc(t.description)+'</div>';});
...
async function renderTicketList(){var c=document.getElementById('ticket-list');if(!c)return;c.innerHTML='<div class="empty-state"><p>Loading…</p></div>';var f={};if(listFilter.mode==='my'&&currentProfile)f.assignedTo=currentProfile.id;if(listFilter.mode==='unassigned')f.unassigned=true;var tickets=await fetchTickets(f);allTicketsCache=tickets||[];var ids=[];(tickets||[]).forEach(function(t){if(t.requester_id)ids.push(t.requester_id);if(t.assigned_to)ids.push(t.assigned_to);});nameCache=Object.assign(nameCache,await fetchProfileNames(ids));if(window.applyTicketFilters){applyTicketFilters(false);return;}if(!tickets.length){c.innerHTML='<div class="empty-state"><p>No tickets.</p></div>';return;}c.innerHTML=tickets.map(function(t){return '<div class="ticket-card" data-id="'+t.id+'"><div><h4>'+esc(t.title)+'</h4><div class="ticket-meta"><span class="ticket-id">'+esc(t.ticket_number)+'</span><span>'+esc(nameCache[t.requester_id]||'Customer')+'</span><span>'+esc(t.category||'')+'</span><span>'+fmt(t.updated_at||t.created_at)+'</span></div></div><div class="badges"><span class="badge '+sc(t.priority)+'">'+esc(t.priority||'')+'</span><span class="badge '+sc(t.status)+'">'+esc(t.status)+'</span></div></div>';}).join('');c.querySelectorAll('.ticket-card').forEach(function(card){card.addEventListener('click',function(){openTicket(card.getAttribute('data-id'));});});}
async function openTicket(id){var t=await fetchTicketById(id);if(!t)return;currentTicketId=id;showAgentView('detail');var pt=document.getElementById('page-title');if(pt)pt.textContent=t.ticket_number;var nameIds=[];if(t.requester_id)nameIds.push(t.requester_id);if(t.assigned_to)nameIds.push(t.assigned_to);var names=await fetchProfileNames(nameIds);var d=document.getElementById('ticket-detail');if(d){d.innerHTML='<h3>'+esc(t.title)+'</h3>'+detailMetaHtml(t,names,true)+'<div class="detail-description">'+esc(t.description)+'</div>';}await refreshAssignDropdown();var ae=document.getElementById('assign-agent');if(ae)ae.value=t.assigned_to||'';var se=document.getElementById('quick-status');if(se)se.value=t.status;var cl=document.getElementById('comments-list');if(cl){cl.innerHTML='<p class="empty-state" style="padding:0.5rem">Loading…</p>';var comments=await fetchComments(id);cl.innerHTML=commentsHtml(comments,true);}}
...
function setupAgent(){
  ...
  var bc=document.getElementById('btn-claim');if(bc)bc.addEventListener('click',async function(){if(!currentTicketId||!currentProfile)return;var r=await updateTicket(currentTicketId,{assigned_to:currentProfile.id,status:'In Progress'});if(r.error){toast(r.error,'error');return;}toast('Claimed','success');openTicket(currentTicketId);renderStats();if(window.applyTicketFilters)window.applyTicketFilters(true);});
  var bs=document.getElementById('btn-save-meta');if(bs)bs.addEventListener('click',async function(){if(!currentTicketId)return;var r=await updateTicket(currentTicketId,{assigned_to:document.getElementById('assign-agent').value||null,status:document.getElementById('quick-status').value});if(r.error){toast(r.error,'error');return;}toast('Updated','success');openTicket(currentTicketId);renderStats();if(window.applyTicketFilters)window.applyTicketFilters(true);});
  var cf=document.getElementById('comment-form');if(cf)cf.addEventListener('submit',async function(e){e.preventDefault();if(!currentTicketId)return;var text=document.getElementById('comment-text').value.trim();if(!text)return;var internal=document.getElementById('comment-internal').checked;var r=await addComment(currentTicketId,text,internal);if(r.error){toast(r.error,'error');return;}document.getElementById('comment-text').value='';document.getElementById('comment-internal').checked=false;toast(internal?'Internal note added':'Update added','success');openTicket(currentTicketId);});
}
