...
async function fetchAgents(){var r=await sb.from('profiles').select('id,full_name').eq('role','agent').order('full_name');return r.error?[]:(r.data||[]);}
...
function showApp(p){hideLoginShowApp();var pc=document.getElementById('portal-customer'),pa=document.getElementById('portal-agent');if(pc)pc.classList.remove('active');if(pa)pa.classList.remove('active');if(p.role==='customer'){if(pc)pc.classList.add('active');var lb=document.getElementById('logged-user-label');if(lb)lb.textContent=p.name+' (Customer)';var w=document.getElementById('cust-welcome-name');if(w)w.textContent=p.name.split(' ')[0];showCustomerTab('submit');}else{if(pa)pa.classList.add('active');var l2=document.getElementById('logged-user-label');if(l2)l2.textContent=p.name+' (Agent)';var an=document.getElementById('agent-name-display');if(an)an.textContent=p.name;refreshAssignDropdown();showAgentView('dashboard');}}
...
async function renderStats(){var tickets=await fetchTickets({});var c={Open:0,'In Progress':0,Waiting:0,Resolved:0,unassigned:0};tickets.forEach(function(t){if(c[t.status]!==undefined)c[t.status]++;if(!t.assigned_to&&t.status!=='Resolved'&&t.status!=='Closed')c.unassigned++;});[['stat-open','Open'],['stat-progress','In Progress'],['stat-waiting','Waiting'],['stat-unassigned','unassigned'],['stat-resolved','Resolved']].forEach(function(x){var el=document.getElementById(x[0]);if(el)el.textContent=c[x[1]]||0;});}
...
function showAgentView(name){document.querySelectorAll('#portal-agent .view').forEach(function(v){v.classList.remove('active');});document.querySelectorAll('#portal-agent .nav-btn').forEach(function(b){b.classList.remove('active');});if(name==='detail'){var dv=document.getElementById('view-detail');if(dv)dv.classList.add('active');return;}currentView=name;var dash=document.getElementById('view-dashboard');if(dash)dash.classList.add('active');var titles={dashboard:'Dashboard','my-tickets':'My Tickets',unassigned:'Unassigned','all-tickets':'All Tickets'};var pt=document.getElementById('page-title');if(pt)pt.textContent=titles[name]||'Dashboard';var nb=document.querySelector('#portal-agent [data-view="'+name+'"]');if(nb)nb.classList.add('active');listFilter.mode=name==='my-tickets'?'my':(name==='unassigned'?'unassigned':'all');renderStats();renderTicketList();}
function setupAgent(){
...
function setupAuthForms(){
  ...
  if(la)la.addEventListener('submit',async function(e){e.preventDefault();clearErrors();if(!requireCloud())return;var r=await signInAgent(document.getElementById('agent-username').value,document.getElementById('agent-password').value);if(r.error){showError('login-agent',r.error);return;}if(r.user.role!=='agent'){await signOut();showError('login-agent','Not an agent account');return;}showApp(r.user);toast('Welcome','success');});
