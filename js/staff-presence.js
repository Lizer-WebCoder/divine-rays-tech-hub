...
(function () {
  ...
  function css() {
    ...
    s.textContent = [
      ...
      '#'+CT+' .emoji-pop{display:none;position:absolute;bottom:calc(100% + 8px);left:.4rem;width:min(300px,calc(100% - .8rem));max-height:200px;overflow:auto;background:#1a1a28;border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:.45rem;box-shadow:0 10px 28px rgba(0,0,0,.5);z-index:30;grid-template-columns:repeat(8,1fr);gap:.1rem}',
      '#'+CT+' .emoji-pop.open{display:grid}',
      '#'+CT+' .emoji-pop button{background:0;border:0;font-size:1.2rem;cursor:pointer;padding:.25rem;border-radius:6px;line-height:1}',
    ...
    if (!document.getElementById(CT)) {
      ...
      document.getElementById('dr-ch-emoji').onclick = function (e) {
        ...
        if (!pop) {
          ...
          var list = '😀😁😂🤣😊😍😘😜🤔😅🙂😉😎🤩😢😭😡😤👍👎❤️🔥🎉✨👏🙏💯✅⭐🚀👀💪🤝🙌😇🥰😆🙄😴🤮'.split('');
          pop.innerHTML = list.map(function(em){ return '<button type="button" data-em="'+em+'">'+em+'</button>'; }).join('');
  ...
  function draw(rows) {
    ...
    box.querySelectorAll('button[data-act]').forEach(function (btn) {
      btn.onclick = function (e) {
        ...
        else if (act === 'cfg-react') {
          var cur = getReacts();
          var next = window.prompt('Set 5 reaction emojis (space separated)\nDefault: ❤️ 😊 😢 😠 👍', cur.join(' '));
          if (next) {
            var parts = next.split(/[\s,]+/).filter(Boolean).slice(0, 5);
            while (parts.length < 5) parts.push(cur[parts.length] || '👍');
            setReacts(parts); toast('Reactions updated', 'success');
            if (active) loadMsg(active.id);
          }
        }
      };
