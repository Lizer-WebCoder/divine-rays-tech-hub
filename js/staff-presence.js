...
(function () {
  ...
  function css() {
    ...
    s.textContent = [
      ...
      '#'+CT+' .bb{max-width:82%;padding:.45rem .65rem;border-radius:12px;font-size:.84rem;line-height:1.4;word-break:break-word;position:relative}',
      ...
      '#'+CT+' .tm-row{display:flex;align-items:center;justify-content:space-between;gap:.35rem;margin-top:.15rem}',
      '#'+CT+' .tm{font-size:.6rem;opacity:.65;flex:1}',
      '#'+CT+' .mm{position:relative;flex-shrink:0}',
      '#'+CT+' .mm-btn{background:transparent;border:none;color:inherit;opacity:.55;cursor:pointer;font-size:.95rem;line-height:1;padding:0 .15rem;border-radius:4px}',
      '#'+CT+' .mm-btn:hover,#'+CT+' .mm.open .mm-btn{opacity:1;background:rgba(0,0,0,.2)}',
      '#'+CT+' .mm-menu{display:none;position:absolute;right:0;bottom:calc(100% + 4px);min-width:150px;background:#1a1a28;border:1px solid rgba(255,255,255,.12);border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,.45);padding:.25rem;z-index:5}',
  ...
  function draw(rows) {
    ...
    box.innerHTML = rows.map(function (m) {
      ...
      return '<div class="bb '+(mine?'me':'them')+'" data-mid="'+esc(m.id)+'">'+esc(m.body)+
        '<div class="tm-row">' +
          '<span class="tm">'+esc(fmt(m.created_at))+(m.edited_at ? ' · edited' : '')+'</span>' +
          '<div class="mm">' +
            '<button type="button" class="mm-btn" aria-label="Message options" title="Options">⋯</button>' +
