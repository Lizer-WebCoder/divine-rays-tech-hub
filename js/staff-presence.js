...
(function () {
  ...
  function css() {
    ...
    s.textContent = [
      ...
      '#'+CT+' .mwrap{display:flex;align-items:center;gap:.25rem;position:relative}',
      ...
      '#'+CT+' .mm-menu{display:none;position:absolute;bottom:calc(100% + 4px);min-width:120px;background:#1a1a28;border:1px solid rgba(255,255,255,.12);border-radius:8px;box-shadow:0 6px 16px rgba(0,0,0,.4);padding:.15rem;z-index:5}',
      '#'+CT+' .mr.me .mm-menu{right:0}',
      '#'+CT+' .mr.them .mm-menu{left:0}',
      '#'+CT+' .mm.open .mm-menu{display:block}',
      '#'+CT+' .mm-menu button{display:block;width:100%;text-align:left;background:transparent;border:none;color:#e8e8f0;font-size:.68rem;padding:.25rem .4rem;border-radius:5px;cursor:pointer;font:inherit}',
      ...
      '#'+CT+' .cp{display:flex;align-items:center;gap:.35rem;padding:.5rem .55rem;border-top:1px solid rgba(255,255,255,.06);position:relative}',
      '#'+CT+' .cp input{flex:1;background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:.55rem .7rem;color:var(--text,#f0f0f8);font:inherit;font-size:.85rem;min-width:0}',
      '#'+CT+' .cp button#dr-ch-send{background:linear-gradient(135deg,#7c6af0,#9b8afb);border:0;border-radius:10px;color:#fff;padding:.55rem .85rem;font-weight:600;cursor:pointer;font:inherit;font-size:.82rem}',
      '#'+CT+' .hn{padding:.75rem;text-align:center;font-size:.8rem;color:#8b8ba3}',
      '#'+CT+' .emoji-pop{display:none;position:absolute;bottom:calc(100% + 8px);left:.4rem;width:min(320px,calc(100% - .8rem));max-height:280px;overflow:auto;background:#1a1a28;border:1px solid rgba(255,255,255,.12);border-radius:14px;padding:.55rem;box-shadow:0 12px 32px rgba(0,0,0,.55);z-index:30}',
      '#'+CT+' .emoji-pop.open{display:block}',
      '#'+CT+' .emoji-pop button:hover{background:rgba(124,106,240,.25)}',
      '#'+CT+' .react-btn{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#c4c4d4;cursor:pointer;font-size:.85rem;line-height:1;width:24px;height:24px;border-radius:50%;display:grid;place-items:center;padding:0;flex-shrink:0;opacity:0;pointer-events:none;transition:opacity .12s}',
      '#'+CT+' .mr:hover .react-btn,#'+CT+' .react-btn.open{opacity:1;pointer-events:auto}',
      '#'+CT+' .react-btn:hover,#'+CT+' .react-btn.open{background:rgba(124,106,240,.25);color:#fff}',
      '#'+CT+' .react-pop{display:none;position:absolute;bottom:calc(100% + 6px);background:#1a1a28;border:1px solid rgba(255,255,255,.12);border-radius:20px;padding:.25rem .35rem;gap:.15rem;box-shadow:0 8px 20px rgba(0,0,0,.45);z-index:20}',
      '#'+CT+' .react-pop.open{display:flex}',
      '#'+CT+' .mr.me .react-pop{right:0}',
      '#'+CT+' .mr.them .react-pop{left:0}',
      '#'+CT+' .react-pop button{background:0;border:0;font-size:1.15rem;cursor:pointer;padding:.2rem .25rem;border-radius:8px;line-height:1}',
      '#'+CT+' .react-pop button:hover{background:rgba(124,106,240,.25);transform:scale(1.15)}',
      '#'+CT+' .reacts{display:flex;flex-wrap:wrap;gap:.2rem;margin-top:.2rem}',
      '#'+CT+' .reacts .chip{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:.05rem .4rem;font-size:.72rem;cursor:pointer}',
      '#'+CT+' .reacts .chip.on{background:rgba(124,106,240,.28);border-color:rgba(124,106,240,.4)}',
      ...
      'html[data-theme="light"] #'+CT+' .mm-menu,html[data-theme="light"] #'+CT+' .emoji-pop,html[data-theme="light"] #'+CT+' .react-pop{background:#fff;border-color:rgba(15,15,30,.12)}',
  ...
  function ui() {
    ...
    if (!document.getElementById(CT)) {
      ...
      c.innerHTML = '<div class="hd"><button type="button" class="bk" id="dr-ch-back">←</button><div class="tt"><strong id="dr-ch-name">Chat</strong><span id="dr-ch-role">—</span></div><button type="button" class="x" id="dr-ch-close">&times;</button></div><div class="ms" id="dr-ch-msgs"></div><div class="cp"><button type="button" id="dr-ch-emoji" title="Emoji" style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:8px;width:34px;height:34px;cursor:pointer;font-size:1rem;flex-shrink:0">😀</button><button type="button" id="dr-ch-attach" title="Image/Video" style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:8px;width:34px;height:34px;cursor:pointer;font-size:.9rem;flex-shrink:0">📎</button><input type="file" id="dr-ch-file" accept="image/*,video/*" style="display:none"/><input id="dr-ch-input" placeholder="Type a message…" maxlength="2000" style="flex:1;min-width:0"/><button type="button" id="dr-ch-send">Send</button></div>';
      ...
      document.getElementById('dr-ch-emoji').onclick = function (e) {
        ...
        if (!pop) {
          ...
          var list = ['😀','😁','😂','🤣','😊','😍','😘','😜','🤔','😅','🙂','😉','😎','🤩','😢','😭','😡','😤','👍','👎','❤️','🔥','🎉','✨','👏','🙏','💯','✅','⭐','🚀','👀','💪','🤝','🙌','😇','🥰','😆','🙄','😴','🤮','🥳','🥺','😤','😠','🤬','😷','🤒','🤕','🤢','🥵','🥶','😵','🤯','🤠','🤑','🤓','🧐','😈','👻','💀','🤖','💩','😺','😸','😹','😻','👊','✊','🙌','👐','🤲','🧡','💛','💚','💙','💜','🖤','🤍','💔','❣️','💕','💞','💓','💗','💖','💘','💝','🌟','⚡','💥','🎊','🎈','🎁','🏆','❌','🔔'];
          pop.innerHTML = '<div style="display:grid;grid-template-columns:repeat(8,1fr);gap:2px">'+list.map(function(em){ return '<button type="button" data-em="'+em+'" style="background:0;border:0;font-size:1.35rem;cursor:pointer;padding:4px;border-radius:8px;line-height:1;font-family:Apple Color Emoji,Segoe UI Emoji,Noto Color Emoji,sans-serif">'+em+'</button>'; }).join('')+'</div>';
          document.querySelector('#'+CT+' .cp').appendChild(pop);
          pop.querySelectorAll('button[data-em]').forEach(function(b){
            b.onclick = function(ev){
              ev.preventDefault(); ev.stopPropagation();
              var i = document.getElementById('dr-ch-input');
              if (i) { i.value += b.getAttribute('data-em'); i.focus(); }
      ...
      document.getElementById('dr-ch-attach').onclick = function () { document.getElementById('dr-ch-file').click(); };
      document.getElementById('dr-ch-file').onchange = async function (ev) {
        var file = ev.target.files && ev.target.files[0];
        ev.target.value = '';
        if (!file || !active) return;
        if (file.size > 6*1024*1024) { toast('Max 6MB', 'error'); return; }
        var client = sb(), my = getMe() && getMe().id; if (!client || !my) return;
        try {
          var path = 'chat/'+my+'/'+Date.now()+'_'+file.name.replace(/[^a-zA-Z0-9._-]/g,'_');
          var up = await client.storage.from('chat-media').upload(path, file, { contentType: file.type });
          var url = null;
          if (!up.error) {
            var pub = client.storage.from('chat-media').getPublicUrl(path);
            url = pub && pub.data && pub.data.publicUrl;
          }
          if (!url && file.type.indexOf('image/')===0 && file.size < 900000) {
            url = await new Promise(function(res,rej){ var r=new FileReader(); r.onload=function(){res(r.result)}; r.onerror=rej; r.readAsDataURL(file); });
          }
          if (!url) { toast(up.error ? up.error.message : 'Create public Storage bucket: chat-media', 'error'); return; }
          var isVid = file.type.indexOf('video/')===0;
          var body = (document.getElementById('dr-ch-input').value||'').trim() || (isVid?'🎥 Video':'🖼️ Image');
          var payload = { sender_id: my, recipient_id: active.id, body: body + '\n' + url, channel_id: newUuid() };
          var r = await client.from('messages').insert(payload).select('*').maybeSingle();
          if (r.error) { delete payload.channel_id; r = await client.from('messages').insert(payload).select('*').maybeSingle(); }
          if (r.error) { toast(r.error.message||'Send failed','error'); return; }
  ...
  function getReacts() {
    try {
      ...
      if (Array.isArray(a) && a.length >= 4) return a.slice(0, 5);
    } catch (e) {}
    return ['❤️','😊','😢','😠','👍'];
  }
  ...
  function reactMap(mid) {
    try { return JSON.parse(localStorage.getItem('dr_react_' + mid) || '{}') || {}; } catch (e) { return {}; }
  }
  function saveReactMap(mid, map) {
    try { localStorage.setItem('dr_react_' + mid, JSON.stringify(map)); } catch (e) {}
  }
  function applyReact(mid, emoji) {
    var my = getMe() && getMe().id;
    var map = reactMap(mid);
    var list = Array.isArray(map[emoji]) ? map[emoji].slice() : [];
    var key = my || 'me';
    var ix = list.indexOf(key);
    if (ix >= 0) list.splice(ix, 1); else list.push(key);
    if (list.length) map[emoji] = list; else delete map[emoji];
    saveReactMap(mid, map);
  ...
  function openReactPicker() {
    ...
    var list = ['😀','😁','😂','🤣','😊','😍','😘','🥰','😎','🤩','🤔','😅','😉','🙂','😢','😭','😡','😤','😠','🤬','🥺','🥳','😇','🙃','😏','😴','🤮','👍','👎','👏','🙌','🙏','💪','❤️','🧡','💛','💚','💙','💜','🖤','💔','🔥','⭐','✨','🎉','💯','✅','❌','👀','🤝'];
    var pop = document.getElementById('dr-emoji-pop');
    var cp = document.querySelector('#'+CT+' .cp');
    if (!cp) return;
    if (!pop) { pop = document.createElement('div'); pop.id = 'dr-emoji-pop'; pop.className = 'emoji-pop'; cp.appendChild(pop); }
    function paint() {
      pop.innerHTML =
        '<div style="font-size:12px;font-weight:600;color:#a8a8bc;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center"><span>Pick 5 reactions</span><button type="button" id="dr-ep-x" style="background:0;border:0;color:#8b8ba3;cursor:pointer;font-size:16px">&times;</button></div>' +
        '<div style="display:flex;gap:6px;margin-bottom:8px">' +
          slots.map(function(em,i){ return '<button type="button" data-slot="'+i+'" style="width:32px;height:32px;border-radius:8px;border:1px '+(em?'solid rgba(124,106,240,.5)':'dashed rgba(255,255,255,.2)')+';background:rgba(124,106,240,.12);font-size:18px;cursor:pointer;line-height:1;font-family:Apple Color Emoji,Segoe UI Emoji,Noto Color Emoji,sans-serif">'+ (em||'') +'</button>'; }).join('') +
        '</div>' +
        '<div style="display:grid;grid-template-columns:repeat(8,1fr);gap:2px;max-height:180px;overflow:auto">' +
          list.map(function(em){ return '<button type="button" data-em="'+em+'" style="background:0;border:0;font-size:1.35rem;cursor:pointer;padding:4px;border-radius:8px;line-height:1;font-family:Apple Color Emoji,Segoe UI Emoji,Noto Color Emoji,sans-serif">'+em+'</button>'; }).join('') +
        '</div>' +
        '<button type="button" id="dr-ep-save" style="margin-top:8px;width:100%;background:linear-gradient(135deg,#7c6af0,#9b8afb);border:0;border-radius:8px;color:#fff;padding:8px;font-weight:600;cursor:pointer;font-size:13px">Save reactions</button>';
      pop.classList.add('open');
      pop.querySelector('#dr-ep-x').onclick = function(e){ e.stopPropagation(); pop.classList.remove('open'); };
      pop.querySelectorAll('[data-slot]').forEach(function(b){
        b.onclick = function(e){ e.stopPropagation(); slots[+b.getAttribute('data-slot')] = ''; paint(); };
      });
      ...
      pop.querySelector('#dr-ep-save').onclick = function(e){
        e.stopPropagation();
        var final = slots.filter(Boolean).slice(0,5);
        var def = ['❤️','😊','😢','😠','👍'];
        while (final.length < 5) final.push(def[final.length]);
        setReacts(final);
        toast('Reactions updated', 'success');
        pop.classList.remove('open');
        if (active) loadMsg(active.id);
      };
    }
  ...
  function draw(rows) {
    ...
    box.innerHTML = rows.map(function (m) {
      ...
      var menuItems = mine
        ? '<button type="button" data-act="edit" data-id="'+esc(m.id)+'" data-body="'+esc(m.body)+'">Edit</button><button type="button" data-act="del-me" data-id="'+esc(m.id)+'">Delete for me</button><button type="button" class="danger" data-act="del-all" data-id="'+esc(m.id)+'">Delete for everyone</button><button type="button" data-act="cfg-react">Customize reacts</button>'
        : '<button type="button" data-act="del-me" data-id="'+esc(m.id)+'">Delete for me</button><button type="button" data-act="cfg-react">Customize reacts</button>';
      var menu = '<div class="mm"><button type="button" class="mm-btn" title="Options">⋯</button><div class="mm-menu">'+menuItems+'</div></div>';
      var reactBtn = '<div class="mm react-wrap" style="position:relative"><button type="button" class="react-btn" data-mid="'+esc(m.id)+'" title="React">😊</button><div class="react-pop">'+
        reacts.map(function (em) { return '<button type="button" data-quick="'+esc(em)+'" data-mid="'+esc(m.id)+'">'+em+'</button>'; }).join('') +
        '</div></div>';
      var rmap = reactMap(m.id);
      var chips = '';
      Object.keys(rmap).forEach(function (em) {
        var users = rmap[em] || [];
        if (!users.length) return;
        var on = my && users.indexOf(my) >= 0;
        chips += '<button type="button" class="chip'+(on?' on':'')+'" data-quick="'+esc(em)+'" data-mid="'+esc(m.id)+'">'+em+' '+users.length+'</button>';
      });
      return '<div class="mr '+(mine?'me':'them')+'" data-mid="'+esc(m.id)+'">'+msgAvatar(who)+'<div class="col"><div class="nm">'+esc(name)+'</div><div class="mwrap">'+
        (function(){
          var text = m.body || ''; var media = '';
          var urls = text.match(/https?:\/\/\S+|data:image\/[^\s]+/g) || [];
          urls.forEach(function(u){
            text = text.replace(u, '').trim();
            if (/\.(mp4|webm|mov)(\?|$)/i.test(u)) media += '<video controls style="max-width:100%;max-height:160px;border-radius:8px;margin-top:4px;display:block" src="'+esc(u)+'"></video>';
            else if (/^data:image\//.test(u) || /\.(png|jpe?g|gif|webp)(\?|$)/i.test(u) || /\/storage\//i.test(u)) media += '<img style="max-width:100%;max-height:160px;border-radius:8px;margin-top:4px;display:block" src="'+esc(u)+'" alt=""/>';
            else media += '<a href="'+esc(u)+'" target="_blank" rel="noopener" style="font-size:.75rem;color:#a78bfa">Attachment</a>';
          });
          return '<div class="bb '+(mine?'me':'them')+'">'+esc(text)+media+'</div>';
        })()+reactBtn+menu+'</div>'+(chips?'<div class="reacts">'+chips+'</div>':'')+'<div class="tm">'+esc(fmt(m.created_at))+'</div></div></div>';
    }).join('');
    box.querySelectorAll('.mm-btn').forEach(function (btn) {
      btn.onclick = function (e) {
        e.preventDefault(); e.stopPropagation();
        var wrap = btn.parentElement; var open = wrap.classList.contains('open');
        box.querySelectorAll('.mm.open').forEach(function (el) { el.classList.remove('open'); });
        box.querySelectorAll('.react-pop.open').forEach(function (el) { el.classList.remove('open'); });
        box.querySelectorAll('.react-btn.open').forEach(function (el) { el.classList.remove('open'); });
    ...
    box.querySelectorAll('.react-btn').forEach(function (btn) {
      btn.onclick = function (e) {
        e.preventDefault(); e.stopPropagation();
        var wrap = btn.parentElement; var pop = wrap.querySelector('.react-pop');
        var open = pop && pop.classList.contains('open');
        box.querySelectorAll('.mm.open').forEach(function (el) { el.classList.remove('open'); });
        box.querySelectorAll('.react-pop.open').forEach(function (el) { el.classList.remove('open'); });
        box.querySelectorAll('.react-btn.open').forEach(function (el) { el.classList.remove('open'); });
        if (!open && pop) { pop.classList.add('open'); btn.classList.add('open'); }
      };
    });
    box.querySelectorAll('button[data-quick]').forEach(function (btn) {
      btn.onclick = function (e) {
        e.preventDefault(); e.stopPropagation();
        applyReact(btn.getAttribute('data-mid'), btn.getAttribute('data-quick'));
      };
    });
    box.querySelectorAll('button[data-act]').forEach(function (btn) {
      btn.onclick = function (e) {
        ...
        else if (act === 'cfg-react') openReactPicker();
      };
    });
  ...
  async function send() {
    ...
    try {
      ...
      if (r.error) { delete payload.channel_id; r = await c.from('messages').insert(payload).select('*').maybeSingle(); }
  ...
  async function unread() {
    ...
    try {
      ...
      (r.data || []).forEach(function (row) {
        ...
        map[row.sender_id] = (map[row.sender_id] || 0) + 1; total++;
      });
      unreadBySender = map;
  ...
  document.addEventListener('click', function (e) {
    if (!(e.target.closest && e.target.closest('.mm'))) {
      document.querySelectorAll('#'+CT+' .mm.open').forEach(function (el) { el.classList.remove('open'); });
    }
    if (!(e.target.closest && (e.target.closest('.react-wrap') || e.target.closest('.react-pop')))) {
      document.querySelectorAll('#'+CT+' .react-pop.open').forEach(function (el) { el.classList.remove('open'); });
      document.querySelectorAll('#'+CT+' .react-btn.open').forEach(function (el) { el.classList.remove('open'); });
    }
    if (!(e.target.closest && (e.target.closest('.emoji-pop') || e.target.id === 'dr-ch-emoji'))) {
      var ep = document.getElementById('dr-emoji-pop');
