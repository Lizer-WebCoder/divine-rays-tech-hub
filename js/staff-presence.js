...
(function () {
  ...
  function css() {
    ...
    s.textContent = [
      ...
      '#'+CT+' .ma{display:flex;flex-wrap:wrap;gap:.25rem;margin-top:.35rem;opacity:0;transition:opacity .15s}',
      '#'+CT+' .bb:hover .ma{opacity:1}',
      '#'+CT+' .ma button{background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.1);color:#c4c4d4;font-size:.62rem;padding:.15rem .35rem;border-radius:6px;cursor:pointer;font:inherit}',
      '#'+CT+' .ma button:hover{border-color:rgba(124,106,240,.45);color:#fff}',
    ...
    box.innerHTML = rows.map(function (m) {
      ...
      if (mine) {
        acts = '<div class="ma">' +
          ...
          '<button type="button" data-act="del-all" data-id="'+esc(m.id)+'">Delete for everyone</button>' +
        '</div>';
      } else {
        acts = '<div class="ma"><button type="button" data-act="del-me" data-id="'+esc(m.id)+'">Delete for me</button></div>';
      }
      return '<div class="bb '+(mine?'me':'them')+'" data-mid="'+esc(m.id)+'">'+esc(m.body)+
        '<div class="tm">'+esc(fmt(m.created_at))+(m.edited_at ? ' · edited' : '')+'</div>'+acts+'</div>';
    }).join('');
