...
(function () {
  ...
  function css() {
    ...
    s.textContent = [
      ...
      '#'+CT+' .cp{display:flex;align-items:center;gap:.35rem;padding:.5rem .55rem;border-top:1px solid rgba(255,255,255,.06)}',
  ...
  function ui() {
    ...
    if (!document.getElementById(CT)) {
      ...
      document.getElementById('dr-ch-emoji').onclick = function () {
        var em = window.prompt('Type or paste emoji(s) to insert');
        if (em) { var i = document.getElementById('dr-ch-input'); i.value += em; i.focus(); }
      };
    ...
    box.innerHTML = rows.map(function (m) {
      ...
      if (mine) {
        menuItems =
          ...
          '<button type="button" data-act="cfg-react">Customize reacts</button>';
      } else {
        menuItems = '<button type="button" data-act="del-me" data-id="'+esc(m.id)+'">Delete for me</button><button type="button" data-act="cfg-react">Customize reacts</button>';
      }
      var menu =
        '<div class="mm">' +
    ...
    box.querySelectorAll('button[data-act]').forEach(function (btn) {
      btn.onclick = function (e) {
        ...
        if (act === 'edit') editMsg(id, btn.getAttribute('data-body') || '');
          ...
          var cur = ['👍','❤️','😂','😮','🔥'];
          try { var a = JSON.parse(localStorage.getItem('dr_react_emojis')||'null'); if (Array.isArray(a)&&a.length) cur=a; } catch(ex){}
          var next = window.prompt('5 reaction emojis (space separated)', cur.join(' '));
          if (next) {
            var parts = next.split(/[\s,]+/).filter(Boolean).slice(0,5);
            while (parts.length<5) parts.push(cur[parts.length]||'👍');
            localStorage.setItem('dr_react_emojis', JSON.stringify(parts));
            toast('Reactions saved','success');
          }
        }
      };
    });
    box.querySelectorAll('.bb').forEach(function(bb){
      bb.ondblclick = function(e){
        e.preventDefault();
        var mid = bb.closest('[data-mid]') && bb.closest('[data-mid]').getAttribute('data-mid');
        if (!mid) return;
        var cur = ['👍','❤️','😂','😮','🔥'];
        try { var a = JSON.parse(localStorage.getItem('dr_react_emojis')||'null'); if (Array.isArray(a)&&a.length) cur=a; } catch(ex){}
        var pick = window.prompt('React with: ' + cur.join('  ') + '\nType one emoji');
        if (!pick) return;
        pick = pick.trim();
        var key = 'dr_react_'+mid;
        var map = {};
        try { map = JSON.parse(localStorage.getItem(key)||'{}')||{}; } catch(ex){}
        var uid = getMe() && getMe().id;
        var list = map[pick] || [];
        if (uid) {
          var ix = list.indexOf(uid);
          if (ix>=0) list.splice(ix,1); else list.push(uid);
        } else list.push('me');
        if (list.length) map[pick]=list; else delete map[pick];
        localStorage.setItem(key, JSON.stringify(map));
        var col = bb.closest('.col');
        if (col) {
          var old = col.querySelector('.reacts');
          if (old) old.remove();
          var html = '';
          Object.keys(map).forEach(function(em){ if(map[em]&&map[em].length) html += '<span style="background:rgba(255,255,255,.08);border-radius:10px;padding:1px 6px;font-size:12px;margin-right:3px">'+em+' '+map[em].length+'</span>'; });
          if (html) { var d = document.createElement('div'); d.className='reacts'; d.style.marginTop='4px'; d.innerHTML=html; col.appendChild(d); }
        }
      };
    });
    box.querySelectorAll('[data-mid]').forEach(function(row){
      var mid = row.getAttribute('data-mid');
      try {
        var map = JSON.parse(localStorage.getItem('dr_react_'+mid)||'null');
        if (!map) return;
        var col = row.querySelector('.col');
        if (!col) return;
        var html='';
        Object.keys(map).forEach(function(em){ if(map[em]&&map[em].length) html += '<span style="background:rgba(255,255,255,.08);border-radius:10px;padding:1px 6px;font-size:12px;margin-right:3px">'+em+' '+map[em].length+'</span>'; });
        if (html) { var d=document.createElement('div'); d.className='reacts'; d.style.marginTop='4px'; d.innerHTML=html; col.appendChild(d); }
